...
// SpecialNiftBar with Governance.
contract SpecialNiftBar is BEP20("SpecialNiftBar Token", "SPECIAL NIFT") {
    ...
    constructor(NiftToken _nift) public {
        nift = _nift;
    }

    // Safe nift transfer function, just in case if rounding error causes pool to not have enough NIFTs.
    function safeNiftTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 niftBal = nift.balanceOf(address(this));
        if (_amount > niftBal) {
            nift.transfer(_to, niftBal);
        } else {
            nift.transfer(_to, _amount);
        }
    }
